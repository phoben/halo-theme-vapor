...
(function (global) {
  ...
  const commentWidget = {
    ...
    getUserWebsiteLink(website, displayName) {
      ...
      if (website && website !== "") {
        ...
        html += `<a target="_blank" href="${escapeHTML(
          website
        )}" class="url hover:underline" rel="ugc external nofollow">${escapedDisplayName}</a>`;
      } else {
    ...
    buildCommentItem(comment) {
      ...
      const escapedRaw = comment.spec.raw;
      let html = `<li id="comment-${commentName}" class="comment depth-1 fade-before relative list-none p-0 m-0 pb-8"> \
        ...
        <div id="div-comment-${commentName}" class="comment-content comment__message mb-[1em] text-sm -mt-4 pl-[3.25rem]">${marked.parse(
        escapedRaw
      )}</div>`;
    ...
    getReplyItem(replies, commentOwnerName, commentName) {
      ...
      for (let i = 0; i < replies.items.length; i++) {
        ...
        const escapedRaw = escapeHTML(reply.spec.raw);
        htmlArr.push(
          ...
          `${
            !quoteReply || !quoteReplys
              ? `<a class="comment-at has-line-clamp thyuu-icon-at" role="button" href="#comment-${commentName}">${commentOwnerName}</a>`
              : `<a class="comment-at has-line-clamp thyuu-icon-at" role="button" href="#reply-${quoteReply}">${quoteReplys?.spec.owner.displayName}</a>`
          }
          <div class="comment-meta">`,
          `${
            commentState.currentUser || commentState.allowAnonymousComments
          ...
          `</div>`,
          `</article>`,
          `<div id="div-comment-${replyName}" class="comment-content mb-1 text-sm pl-8">${marked.parse(
            escapedRaw
          )}</div>`,
          `</li>`
        );
    ...
    async replyForm() {
      ...
      const replyRequest = { raw: content, content: content, allowNotification: false };
